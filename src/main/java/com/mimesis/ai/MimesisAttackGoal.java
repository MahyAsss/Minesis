...
public class MimesisAttackGoal extends Goal {
    ...
    public boolean canUse() {
        return this.mimesis.isHostile() || this.mimesis.isProvokedByPlayer();
    }
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (distance > 2.8D) {
                ...
                this.mimesis.getMoveControl().setWantedPosition(target.getX(), target.getY(), target.getZ(), this.mimesis.isHostile() ? 1.35D : 1.05D);
    ...
    private Player resolveTarget() {
        Player target = this.mimesis.getTarget() instanceof Player ? (Player) this.mimesis.getTarget() : null;
        if (target != null && target.isAlive()) {
            this.mimesis.setTarget(target);
            return target;
        }

        target = this.mimesis.level().getNearestPlayer(
                this.mimesis.getX(),
                this.mimesis.getY(),
                this.mimesis.getZ(),
                this.mimesis.isHostile() ? 256.0D : 96.0D,
                false);
    ...
    private void equipSword() {
        ...
        if (!mainHand.getItem().toString().toLowerCase().contains("sword") || mainHand.getItem() != Items.IRON_SWORD) {
            this.mimesis.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(Items.IRON_SWORD));
        }
    }
