 ...
 */
public class MinesisBlockBreakGoal extends Goal {
    private final MinesisEntity minesis;
    private BlockPos targetBlock = null;
    private int breakProgress = 0;
    private int scanTimer = 0;

    public MinesisBlockBreakGoal(MinesisEntity minesis) {
    ...
    public void tick() {
        ...
        double distSq = this.minesis.blockPosition().distSqr(this.targetBlock);
        ...
        if (distSq > 16) {
            ...
            this.minesis.setDeltaMovement(0, this.minesis.getDeltaMovement().y, 0);
            
            // Simple distance check for direct line of sight
            double blockDist = this.minesis.distanceToSqr(Vec3.atCenterOf(this.targetBlock));
            ...
            if (this.minesis.level() instanceof ServerLevel serverLevel) {
                serverLevel.destroyBlockProgress(this.minesis.getId(), this.targetBlock, Math.min(9, this.breakProgress / 2));
            }
            ...
            if (this.breakProgress >= 12) {
                ...
                if (this.minesis.level() != null && !this.minesis.level().isClientSide) {
                        ...
                        if (this.minesis.level() instanceof ServerLevel serverLevel) {
                            ...
                            for (ItemEntity item : serverLevel.getEntitiesOfClass(ItemEntity.class, box)) {
                                if (item != null && !item.isRemoved()) {
                                    ...
                                    if (!stack.isEmpty()) {
                                        this.minesis.addToCarriedItems(stack.copy());
                                    }
                                    item.remove(net.minecraft.world.entity.Entity.RemovalReason.DISCARDED);
                                }
                            }

                            serverLevel.destroyBlockProgress(this.minesis.getId(), this.targetBlock, -1);
                        }
                    }
                this.stop();
            }
    ...
    private void equipToolForBlock(BlockPos blockPos) {
        Block block = this.minesis.level().getBlockState(blockPos).getBlock();
        ...
        this.minesis.setItemSlot(EquipmentSlot.MAINHAND, mainHand);
    }
    ...
    @Override
    public void stop() {
        if (this.targetBlock != null && this.minesis.level() instanceof ServerLevel serverLevel) {
            serverLevel.destroyBlockProgress(this.minesis.getId(), this.targetBlock, -1);
        }
    ...
    private BlockPos findNearestOre() {
        BlockPos center = this.minesis.blockPosition();
    ...
    private boolean canReachTargetBlock() {
        ...
        return this.minesis.getNavigation().createPath(this.targetBlock, 0) != null;
    }
    ...
    private boolean hasLineOfSightTo(BlockPos blockPos) {
        ...
        BlockHitResult hit = this.minesis.level().clip(new ClipContext(
                start,
                end,
